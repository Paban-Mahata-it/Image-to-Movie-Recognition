...
def main():
    ...
    for movie_name in os.listdir(FRAMES_DIR):
        movie_folder = os.path.join(FRAMES_DIR,movie_name)

        if not os.path.isdir(movie_folder):
            continue

        # sort the frames by their names
        frame_files = sorted(os.listdir(movie_folder))

        for idx, frame_file in enumerate(frame_files):
            frame_path = os.path.join(movie_folder, frame_file)
            timestamp = get_frame_timestamp(idx, fps=0, interval=1)

            rows.append([movie_name,frame_path,timestamp])
